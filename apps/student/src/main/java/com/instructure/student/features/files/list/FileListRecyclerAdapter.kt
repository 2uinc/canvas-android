...
open class FileListRecyclerAdapter(
    ...
    override fun bindHolder(item: FileFolder, holder: FileViewHolder, position: Int) {
        holder.bind(item, contextColor, context, FileListFragment.getFileMenuOptions(item, canvasContext, fileListRepository.isOnline(), folder), fileFolderCallback, canvasContext.id)
    }
